...
**核心概念**：
- **VPN 端點關聯費用**：每個關聯的子網路收費 **USD 0.10/小時**
- **閒置檢測**：當 VPN 無人連線超過設定時間（預設 54 分鐘，已優化）時視為閒置
- **自動關閉**：系統自動解除子網路關聯，停止計費
- **即時重啟**：需要時可立即透過 `/vpn open` 指令重新啟用

#### 📊 實際節費效果

**每日節費範例**：
```
情境：VPN 每天使用 5 小時，閒置 19 小時
...
💵 Cost Savings: $0.10/hour (~$0.10 saved for idle period)
