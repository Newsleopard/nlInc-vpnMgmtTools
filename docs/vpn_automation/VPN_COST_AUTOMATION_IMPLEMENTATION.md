...
- 建議啟用 Lambda X-Ray 追蹤：
  `lambdaFn.addEnvironment('AWS_XRAY_SDK_ENABLED','true')`

## 12. Extension Points

1. **新增 Slack 指令**：
   - 編輯 `lambda/slack-handler/router.ts` switch-case
   - 在 `lambda/vpn-control` 實作對應 action
2. **自訂指標**：透過 `cloudwatch:PutMetricData` 上報
3. **擴充環境**：於 CDK stack 新增環境變數與資源
4. **跨帳號整合**：
   - 設定 STS AssumeRole 權限
   - 更新 slack-handler 的路由邏輯
   - 配置目標帳號的 Lambda ARN 環境變數
5. **多區域支援**：可擴展為支援不同 AWS 區域的 VPN 端點管理
