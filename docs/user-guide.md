...
## ⏰ VPN 自動化排程說明 | Automation Schedule

### 排程總覽 | Schedule Overview

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  每日排程流程 (台灣時間 UTC+8) - Production 環境                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  10:00 AM ──┬── ⏰ 自動開啟 (Auto-Open)                                      │
...
│             │   閒置偵測開始生效                                              │
│             │                                                               │
│   5:00 PM+  │   ⏱️ 閒置偵測 (Idle Detection)                                 │
│      ↓      │   • 客戶端：100 分鐘無流量 → 自動斷線                            │
│             │   • 伺服器端：30 分鐘無連線 → 自動關閉端點                         │
│             │                                                               │
│  ~7:10 PM ──┴── 🔴 典型自動關閉時間                                          │
│                 (假設 5:00 PM 後無使用)                                       │
│                                                                             │
...
週末排程 (Friday)：
┌─────────────────────────────────────────────────────────────────────────────┐
│  週五 8:00 PM ── 🌙 週末軟關閉 (Soft-Close)                                   │
│                  │                                                          │
│                  ├── 無連線 → 立即關閉                                        │
│                  │                                                          │
│                  └── 有連線 → 等待 30 分鐘後重試                               │
│                              持續重試直到所有連線結束                           │
│                              Slack 通知會顯示連線中的使用者名單                  │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 環境預設值 | Environment Defaults

| 設定 | Production | Staging |
|------|------------|---------|
| 自動開啟 (Auto-Open) | ✅ 預設啟用 (10:00 AM) | ❌ 預設關閉 |
| 自動關閉 (Auto-Close) | ❌ 預設關閉 | ❌ 預設關閉 |
| 營業時間保護 | ✅ 10:00 AM - 5:00 PM | ✅ 10:00 AM - 5:00 PM |
| 週末軟關閉 | ✅ 週五 8:00 PM | ✅ 週五 8:00 PM |

### 保護機制說明 | Protection Mechanisms

| 保護機制 | 適用範圍 | 說明 |
|----------|----------|------|
| 🛡️ 營業時間保護 | 僅自動關閉 | 10:00 AM - 5:00 PM 期間不會自動關閉 |
| ⏸️ 15 分鐘寬限期 | 僅自動關閉 | 手動 `/vpn open` 或 `/vpn close` 後 15 分鐘內不會自動關閉 |
| 🌙 週末軟關閉 | 週五 8:00 PM | 有連線時等待，不強制斷線 |
...
| 層級 | 觸發條件 | 超時時間 | 行為 |
|------|----------|----------|------|
| 客戶端 | 100 分鐘無實際流量 | 100 分鐘 | OpenVPN 客戶端自動斷線 |
| 伺服器端 | 30 分鐘無任何連線 | 30 分鐘 | Lambda 自動關閉 VPN 端點 |

**重要說明：**
- ✅ SSH、HTTP、資料庫查詢等實際資料傳輸會重置計時器
- ❌ VPN keepalive 封包不會重置計時器
- 伺服器端檢查每 3 分鐘執行一次
