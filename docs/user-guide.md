...
| 保護機制 | 適用環境 | 說明 |
|----------|----------|------|
| 🛡️ 營業時間保護 | **僅 Production** | 10:00 AM - 5:00 PM 期間不會自動關閉 (Staging 無此保護) |
| ⏸️ 15 分鐘寬限期 | 兩環境皆適用 | 手動 `/vpn open` 或 `/vpn close` 後 15 分鐘內不會自動關閉 |
| 🌙 週末軟關閉 | 兩環境皆適用 | 週五 8:00 PM，有連線時等待，不強制斷線 |
