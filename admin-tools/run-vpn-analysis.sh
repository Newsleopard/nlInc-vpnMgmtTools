...
echo "🔧 VPN Analysis Environment-Aware Tool"
echo "======================================"
...
load_configuration() {
    ...
    if [ -f "$ADMIN_CONFIG_FILE" ]; then
        echo "📄 Loading admin configuration: $ADMIN_CONFIG_FILE"
        source "$ADMIN_CONFIG_FILE"
    fi
    
    # Load environment-specific configuration
    if [ -f "$CONFIG_FILE" ]; then
        echo "📄 Loading environment configuration: $CONFIG_FILE"
        source "$CONFIG_FILE"
    else
        echo -e "${YELLOW}⚠️  Environment configuration not found: $CONFIG_FILE${NC}"
...
SUBNET_ID=
EOF
        echo "📄 Created template configuration: $CONFIG_FILE"
    ...
    if [ -z "$VPC_ID" ] || [ "$VPC_ID" = "vpc-d0f3e2ab" ]; then
        echo -e "${YELLOW}⚠️  Using default VPC ID. Please update $CONFIG_FILE with your actual VPC ID.${NC}"
    fi
}

# Function: Test environment and prerequisites
test_environment() {
    echo -e "${BLUE}🧪 Testing VPN Analysis Environment${NC}"
    echo "======================================"
    echo "Environment: $ENVIRONMENT"
    echo "Region: $REGION"
    echo "VPC: $VPC_ID"
    ...
    if [ "$BASH_VERSION_MAJOR" -lt 4 ]; then
        echo -e "${RED}❌ Bash version 4.0+ required for associative arrays. Current: $BASH_VERSION${NC}"
        echo ""
    ...
    echo -e "${GREEN}✅ Bash: $BASH_VERSION (supports associative arrays)${NC}"
    ...
    echo -e "${GREEN}✅ AWS CLI: $(aws --version | head -1)${NC}"
    ...
    if [ "$VPN_COUNT" -eq 0 ]; then
        echo -e "${YELLOW}  ⚠️  No VPN endpoints found in $ENVIRONMENT environment.${NC}"
        echo -e "${YELLOW}  💡 You can create one using: $PROJECT_ROOT/aws_vpn_admin.sh create-endpoint $ENVIRONMENT${NC}"
    else
        echo -e "${GREEN}  ✅ VPN endpoints available for analysis in $ENVIRONMENT environment${NC}"
    fi
    echo ""
...
5. **🔄 Redundant Coverage**: Multiple paths to the same services for reliability
6. **🏗️ Future-Proof**: Well-architected for easy maintenance and expansion
...
display_results() {
    echo ""
    echo -e "${GREEN}✅ Analysis completed successfully for $ENVIRONMENT!${NC}"
    echo ""
    
    # Display summary from JSON with environment context
    if [ -f "$OUTPUT_FILE" ]; then
        echo -e "${CYAN}📋 Quick Summary ($ENVIRONMENT Environment):${NC}"
        echo "================================================="
        ...
        echo "🏷️  Environment: $ENV_RESULT"
        echo "🕐 Analysis Time: $TIMESTAMP"
        echo "🌐 VPC: $VPC_ID_RESULT"
        echo "🔒 Security Groups with VPN Access: $TOTAL_SG"
        echo "🔌 Total Accessible Ports: $TOTAL_PORTS"
        echo "📊 Overall Status: $OVERALL_STATUS"
        echo ""
        
        echo -e "${CYAN}🎯 Accessible Services:${NC}"
        ...
        echo -e "${CYAN}📡 VPN Endpoint Details:${NC}"
        jq -r '.vpn_endpoints[] | "  🔗 ID: \(.ClientVpnEndpointId)\n  📊 Status: \(.Status.Code)\n  🌐 CIDR: \(.ClientCidrBlock)\n  🔒 Security Group: \(.SecurityGroupIds[0])"' "$OUTPUT_FILE"
        echo ""
    fi
    
    # Offer to display full report
    echo -e "${YELLOW}📄 Full Report Options:${NC}"
    echo "======================="
    ...
    case $choice in
        1)
            echo ""
            echo -e "${BLUE}📖 Displaying Full Report:${NC}"
            echo "=========================="
...
# Step 3: Run analysis
echo -e "${YELLOW}📊 Running comprehensive VPN analysis for $ENVIRONMENT...${NC}"
echo ""
...
if run_comprehensive_analysis; then
    ...
    echo ""
    echo -e "${RED}❌ Analysis failed for $ENVIRONMENT environment!${NC}"
...
echo ""
echo -e "${GREEN}🎉 VPN Analysis Complete for $ENVIRONMENT!${NC}"
echo ""
echo "Generated files:"
echo "  📄 $REPORT_FILE - Detailed markdown report"
echo "  📊 $OUTPUT_FILE - Structured JSON data"
