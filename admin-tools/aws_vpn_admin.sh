...
選項:
    ...
    --environment ENV     目標環境 (staging|production)
    --help               顯示此幫助訊息

範例:
    $0                              # 互動式 profile 選擇
    $0 --profile production         # 使用 production profile
    $0 --environment staging        # 目標 staging 環境
...
ENV_CONFIG_FILE="$VPN_CONFIG_DIR/${SELECTED_ENVIRONMENT}.env"
ENDPOINT_CONFIG_FILE="$VPN_CONFIG_DIR/vpn_endpoint.conf"
CONFIG_FILE="$ENV_CONFIG_FILE"  # Primary config for creation process
LOG_FILE="$VPN_LOG_DIR/vpn_admin.log"
...
create_vpn_endpoint() {
    ...
    client_vpn_sg_id=$(create_dedicated_client_vpn_security_group "$vpc_id" "$AWS_REGION" "$SELECTED_ENVIRONMENT")
    ...
    if [ $? -ne 0 ] || [ -z "$client_vpn_sg_id" ]; then
        ...
        local vpn_endpoint_conf="$PROJECT_ROOT/configs/${SELECTED_ENVIRONMENT}/vpn_endpoint.conf"
    ...
    local env_config_file="$CONFIG_DIR/${SELECTED_ENVIRONMENT}.env"
    ...
    echo -e "1️⃣ ${BLUE}檢查生成的腳本${NC}：${YELLOW}security_group_commands_${SELECTED_ENVIRONMENT}.sh${NC}"
...
delete_vpn_endpoint() {
    ...
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}環境:${NC} ${SELECTED_ENVIRONMENT}"
    ...
    if [ "$SELECTED_ENVIRONMENT" = "production" ]; then
        echo -e "\\n${RED}⚠️ 生產環境額外確認：${NC}"
...
manage_aws_profiles() {
    while true; do
        ...
        case "$profile_choice" in
            1)
                # 設定當前環境的 AWS Profile
                echo -e "\n${CYAN}=== 設定 $SELECTED_ENVIRONMENT 環境的 AWS Profile ===${NC}"
                local selected_profile
                selected_profile=$(select_profile_interactive "$SELECTED_ENVIRONMENT")
                if [[ $? -eq 0 ]] && [[ -n "$selected_profile" ]]; then
                    # Validate the newly selected profile
                    if validate_profile_account "$selected_profile" "$SELECTED_ENVIRONMENT"; then
                        # Update global variables
                        SELECTED_AWS_PROFILE="$selected_profile"
                        export AWS_PROFILE="$selected_profile"
                        echo -e "\n${GREEN}✅ Profile 設定完成${NC}"
                    else
                        echo -e "\n${RED}錯誤: Profile 驗證失敗${NC}"
                    fi
                else
            ...
            2)
                ...
                validate_profile_account "$SELECTED_AWS_PROFILE" "$SELECTED_ENVIRONMENT"
                echo -e "\n${YELLOW}按任意鍵繼續...${NC}"
                read -n 1
                ;;
            3)
                ...
                for env_dir in "$SCRIPT_DIR/../configs"/*; do
                    if [[ -d "$env_dir" ]]; then
                        ...
                        if [[ -f "$env_file" ]]; then
                            ...
                            local detected_profile=$(get_profile_for_environment "$env_name" 2>/dev/null)
                            if [[ -n "$detected_profile" ]]; then
                                echo -e "  Profile: $detected_profile"
                            else
                                echo -e "  ${YELLOW}未設定 Profile${NC}"
                            fi
                        fi
                    fi
                done
            ...
            5)
                ...
                for env_dir in "$SCRIPT_DIR/../configs"/*; do
                    if [[ -d "$env_dir" ]]; then
                        ...
                        if [[ -f "$env_file" ]]; then
                            echo -e "\n=== $env_name 環境 ==="
                            local env_profile=$(get_profile_for_environment "$env_name" 2>/dev/null)
                            if [[ -n "$env_profile" ]]; then
                                validate_profile_account "$env_profile" "$env_name"
                            else
                                echo -e "${YELLOW}無可用的 Profile 用於 $env_name 環境${NC}"
                            fi
                        fi
                    fi
                done
